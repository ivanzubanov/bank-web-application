...
async def verify_user_otp_endpoint(data: UserVerifySchema, db: AsyncSession = Depends(get_db)):
    return await verify_user_otp(data, db)

@router.post("/verify-otp/resend")
async def user_resend_otp_endpoint(data: OTPResendSchema, db: AsyncSession = Depends(get_db)):
    return await user_resend_otp(data, db)

@router.post("/login", response_model=TokenResponseSchema)
async def login_user_endpoint(data: UserLoginSchema, db: AsyncSession = Depends(get_db)):
